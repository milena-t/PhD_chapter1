...
def plot_slopes(GF_sizes_dict, species_list, exp_dict, x_label, filename = "sig_OGs_inclines.png", color_category = "orthoDB", percentile = 99):
    ...
    ax.text(0.5*max(OG_sizes_list), percentile_upper, f'{percentile_upper:.3f}', color='r', va='top', ha="center", transform=ax.get_yaxis_transform())
    ax.axhline(percentile_lower, linestyle='--', color = colors["background"])
    ax.text(0.5*max(OG_sizes_list), percentile_lower, f'{percentile_lower:.3f}', color='r', va='top', ha="center", transform=ax.get_yaxis_transform())
    
    ylab = f"regression slope (color by {100-percentile}th and {percentile}th percentile, \n{len(inclines)-not_significant_lines} of {len(inclines)} orthogroups)"
    ax.set_ylabel(ylab, fontsize = fs)
    ax.set_xlabel("orthogroup size", fontsize = fs)
    title_ = x_label.split(" in")[0]
    title = f"regression slopes of \ngene family size vs. {title_}"

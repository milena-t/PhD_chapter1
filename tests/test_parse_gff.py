...
class TestFeatureClass(unittest.TestCase):
    ...
    def test_read_annotation(self):
        """All annotation files are read correctly"""
        ...
        for species in native_annotations.keys():
            self.assertTrue(gff.parse_gff3_general(native_annotations[species]))
            self.assertIsInstance(gff.parse_gff3_general(native_annotations[species]), dict)

    def test_utilities(self):
        """
        Test all the utilities in parse_gff
        """
